...
# Package imports (run_scan.py lives inside ethioscan/)
...
class EthioScanOrchestrator:
    ...
    def check_allowlist(self) -> bool:
        ...
        if getattr(self.args, "confirm_allow", "") == "I_HAVE_PERMISSION":
    ...
    async def run_crawler(self) -> Dict[str, Any]:
        ...
        try:
            ...
            console.print(f"  - {len(crawl_results.get('pages', []))} pages")
            console.print(f"  - {len(crawl_results.get('forms', []))} forms")
            console.print(f"  - {len(crawl_results.get('params', []))} parameterized URLs")
    ...
    def generate_test_cases(self, crawl_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if not self.args.lab:
            filtered_tests = [test for test in all_tests if not test.get("meta", {}).get("lab_only", False)]
                ...
                try:
                    ...
                    async with aiohttp.ClientSession(
                        ...
                        if finding:
                            ...
                            console.print(f"[red]Vulnerability found: {finding.get('category')} in {finding.get('param')}[/red]")
    ...
    def save_findings(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Save findings to JSON file and return scan_info dict for DB/reporting use.
        """
        console.print(f"[blue]Saving findings to {self.args.out}[/blue]")

        # Ensure output directory exists (handle case when out has no directory)
        out_dir = os.path.dirname(self.args.out) or "."
        ...
        # Return scan_info for DB saving / reporting
        return scan_info
        ...
        if findings:
            ...
            for i, finding in enumerate(findings[:5], 1):
                summary_lines.append(
                    f"{i}. {finding.get('category','').upper()} - {finding.get('severity','')} - {finding.get('param','')} - {finding.get('url','')}"
                )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            # Save results (and get scan_info)
            scan_info = self.save_findings(findings)
            self.create_summary(findings)

            # Optional: Save to SQLite DB if requested
            if getattr(self.args, "save_db", False):
                try:
                    from ethioscan import db
                    db.initialize_db()
                    scan_id = db.save_scan(scan_info)
                    db.save_findings(scan_id, findings)
                    console.print(f"[green]Saved scan to database (scan_id={scan_id})[/green]")
                except Exception as e:
                    console.print(f"[yellow]Warning: failed to save to DB: {e}[/yellow]")

            # Optional: Generate HTML report if requested (--report-html)
            if getattr(self.args, "report_html", None):
                try:
                    from ethioscan.reporter import generate_report
                    generate_report(self.args.out, self.args.report_html)
                    console.print(f"[green]HTML report generated: {self.args.report_html}[/green]")
                except Exception as e:
                    console.print(f"[yellow]Warning: failed to generate HTML report: {e}[/yellow]")
...
Safety Notice:
  ...
  explicit permission before scanning any target.
    ...
    parser.add_argument(
        "--report-html",
        dest="report_html",
        help="Generate HTML report from findings JSON",
        default=None
    )
    ...
    parser.add_argument(
        ...
        dest="save_db",
        action="store_true",
        help="Save scan results to local SQLite database"
    )
